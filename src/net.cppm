...
inline int shutdown(conn c, shut d) {
    return kal_net_shutdown(c, static_cast<int>(d));
}
...
struct conn_result     { conn     c;  int e; };
struct listener_result { listener l;  int e; };
struct endpoint_result { endpoint ep; int e; };

inline conn_result connect(const endpoint& to) {
    conn c{};
    const int e = kal_net_connect(&to, &c);
    return { c, e };
}
...
inline conn_result accept(listener l) {
    conn c{};
    const int e = kal_net_accept(l, &c);
    return { c, e };
}
...
inline void close(conn     c) { kal_net_close(c); }
