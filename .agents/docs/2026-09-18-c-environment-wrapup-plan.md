...
| compat.* 第三方包 30 成员 Windows 端剩 18 个失败（`archive` / `c-ares` / `capi-lua` / `catch2` / `cli11` / `cmp-module` / `curl` / `doctest` / `eigen` / `fmtlib.fmt` / `gzip-hpp` / `libpng` / `mimalloc` / `re2` / `spdlog` / `sqlite3` / `tinyhttps` / `zlib`） | upstream 源码（compat.* 是上游源码的 mcpp-index 适配 wrapper，没改源码）+ c-abi 设计 trade-off（仅不定义 `_WIN32`，依赖 cygwin triple 预定义 `__unix__`，不主动 `-D__unix__`） | **本轮已交付**: linux 27/3 不变，Windows 0/30→12/30 改善；剩 18 个为 upstream `#ifdef _WIN32` 与 `#elif __unix__` 二选一假设在"Windows × posix 环境"下的缺口。**已开 mcpp#674**（commit 引：根因 + 4 候选路径：leave-as-is 走 mcpp-index per-package patch / 重新测 `__unix__` trade-off / `-include unistd.h` 广播策略 / 测量与广播分离）。具体走哪条由 mcpp 维护者拍，mcpplibs/mcpp-index 等 mcpp 决议后做 per-package 适配 | — |
