...
| 30 成员重测：Linux 27/3 不变，Windows 12/18（vs 0.13 的 0/30） | compat.* 包只 `#ifdef _WIN32` 不 `#elif __unix__`，mcpp 引擎侧（`__unix__` 已由 cygwin triple 预定义）已完成 | **本轮已交付**: linux 不变 + windows 0→12 改善；剩 18 个 compat.* 第三方包为 upstream 代码假设问题，不是引擎问题。已开 mcpp#674 描述根因 + 4 候选路径（leave-as-is / 重新测 `__unix__` trade-off / `-include unistd.h` 策略 / 测量与广播分离），具体走哪条由 mcpp 维护者拍 | — |
