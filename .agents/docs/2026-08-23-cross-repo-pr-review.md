...
**这一条比它看起来重要**:本轮在三台宿主上一共修了九处缺陷,而其中至少三处
...
而 openkal 的 §9 把 conformance 定为**规范的核验程序**,不是可选项。裸机的
...
| # | 维度 | 事项 | 状态 |
|---|---|---|---|
| 01 | B | 删 `examples/import-std/.mcpp/`,并把扫描判据改成不锚定路径开头 | yes |
...
| 15 | B | git 依赖的两个网络步骤都没有重试 | 共用 `run_with_network_retry`;第一次只盖了一半 |
...
这个地址不是任何程序的性质 —— 它是**这段固件把控制权交到哪里**,正是
`openkal-opensbi` 存在的理由。⇒ 归本包,由 `build.mcpp` 的 `link_script` 送到使用者
的链接线上。

而尺寸是这份文件里唯一的判断:打印一个字符串的 C 程序不需要 256 KiB 栈和
...
后果不是错误而是噪声,且是**遮蔽性的**噪声:十九条「参数未被使用」的警告排在
...
### 4.4 B(稳定性):Windows 宿主上没有 include 依赖追踪 —— 记录而不修
...
**爆炸半径**:CI 每次都是干净检出,不受影响;受影响的是 Windows 上的**增量开发
...
| `graph_runtime_compile_flags` | 2 | yes |
| `Triple::llvm_triple` | 4 | yes |

而 `format_for` 此前**不是函数**,是一个一千五百行函数里的 lambda —— 那正是它
...
| 修复 | 复核方式 | 结果 |
|---|---|---|
| 套件的 `main` 改 `extern "C"` | hosted conformance 全量重跑 | 97 held / 0 / 1,与改动前一致;无 `'main' should not be extern "C"` 警告 |
...
| `openkal-llvm-runtime` | 四个目标、真机运行、`PATCHES.md` 全都没有;而授权段写着 vendored 源码「are unchanged」,**那句话现在是假的** —— 台账里记着五处原地标记补丁 |
...
其中「are unchanged」那一条与授权相邻,而它与同一个仓库里的 `llvm/PATCHES.md`
...
**而第一次修复只盖了一半**:给 `git clone` 加了重试,用一个不存在的仓库做探针
...
`openkal-llvm-runtime` 一度在同一个 SHA 上同时挂着一次失败和一次成功 —— 前者是
